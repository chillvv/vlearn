import { ABILITIES, ERROR_TYPES, getErrorTypesBySubject, getKnowledgePointsBySubject, type Question, type Subject } from './types';
...
export function normalizeMistakeDraft(
  ...
  const knowledgePoint = subjectKnowledgePoints.includes(String(draft.knowledge_point))
    ? String(draft.knowledge_point)
    : defaultPoint;
  const ability = dictionary.ability.includes(String(draft.ability))
    ? String(draft.ability)
    : defaultAbility;
  const errorType = subjectErrorTypes.includes(String(draft.error_type))
