...
function normalizeAsciiLabel(input: string) {
  return input
    .replace(/[Ａ-Ｈａ-ｈ]/g, (char) => String.fromCharCode(char.charCodeAt(0) - 65248))
    .replace(/[０-９]/g, (char) => String.fromCharCode(char.charCodeAt(0) - 65248));
}

function mapLabelToLetter(raw: string) {
  const value = raw.toUpperCase();
  if (/^[A-H]$/.test(value)) return value;
  if (/^[1-8]$/.test(value)) return String.fromCharCode(64 + Number(value));
  return '';
}

export function normalizeChoiceAnswerLabel(raw: string) {
  const compact = normalizeAsciiLabel(normalizeText(raw || ''));
  if (!compact) return '';
  const exactLetter = compact.match(/^([A-H])(\.|、|:)?$/i);
  if (exactLetter) return mapLabelToLetter(exactLetter[1]);
  const exactDigit = compact.match(/^([1-8])(\.|、|:)?$/);
  if (exactDigit) return mapLabelToLetter(exactDigit[1]);
  const prefixLetter = compact.match(/^([A-H])[\.\s、:：\-\(（]/i);
  if (prefixLetter) return mapLabelToLetter(prefixLetter[1]);
  const prefixDigit = compact.match(/^([1-8])[\.\s、:：\-\(（]/);
  if (prefixDigit) return mapLabelToLetter(prefixDigit[1]);
  const answerText = compact.match(/(?:答案|正确答案|应选|选择)\s*(?:是|为)?\s*[:：]?\s*([A-H1-8])/i);
  if (answerText) return mapLabelToLetter(answerText[1]);
  const isolatedLabel = compact.match(/(?:^|[\s(（【\[])([A-H1-8])(?:[\s)）】\].,，。!！?？:：]|$)/i);
  if (isolatedLabel) return mapLabelToLetter(isolatedLabel[1]);
  return '';
}
...
export function normalizeCorrectAnswer(raw: string, options: QuestionChoiceOption[]) {
  ...
  const comparable = normalizeComparableText(compact);
  const fuzzyMatchByText = options.find((item) => normalizeComparableText(item.text) === comparable);
  if (fuzzyMatchByText) return fuzzyMatchByText.label;
  
  const normalizedLabel = normalizeChoiceAnswerLabel(compact);
